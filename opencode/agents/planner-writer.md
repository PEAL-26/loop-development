...
2. **Depois do planeamento macro (passo 6 do fluxo):** escreve `plans/<id>/spec.md` com:
...
3. **Depois da geração de tarefas (passo 10 do fluxo):** escreve **um ficheiro por tarefa** em `plans/<id>/tasks/<numero>-<slug>.md` (ex: `001-validar-token.md`), numerados pela ordem de execução, cada um com:
   - Título e objetivo.
...
4. Regista cada decisão relevante tomada durante o planeamento em `plans/<id>/decisions.md`.
